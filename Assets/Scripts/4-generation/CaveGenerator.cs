...
public class CaveGenerator
{
    ...
    public void RandomizeMap()
    {
        ...
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                ...
                {
                    //Random floor, hill, mountains ans sea
                    if (random.NextDouble() < randomFillPercent) bufferOld[x, y] = 0;
                    else
                    {
    ...
    public void SmoothMap()
    {
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                ...
                //Uses bufferOld in order to count the environment
                ...
                //more smoothing rules
                if (floor == 0) bufferNew[x, y] = 1;
                if (hill == 0) bufferNew[x, y] = 2;
                if (mountains == 0) bufferNew[x, y] = 3;
                if (sea == 0) bufferNew[x, y] = 0;
            }
        }
    ...
    //Given a cell, how many of the 8 surrounding cells are from given typ
    private int GetSurroundingByTheSameTypeCount(int cellX, int cellY, int currentNumber)
    {
        ...
        for (int neighborX = cellX - 1; neighborX <= cellX + 1; neighborX++)
        {
            for (int neighborY = cellY - 1; neighborY <= cellY + 1; neighborY++)
            {
                ...
                //This neighbor is same as the given typ 
