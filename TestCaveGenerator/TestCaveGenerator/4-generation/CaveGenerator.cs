        ...
        for (int x = 0; x < gridSize; x++) {
            for (int y = 0; y < gridSize; y++) {
                if (x == 0 || x == gridSize - 1 || y == 0 || y == gridSize - 1) {
                    ...
                    bufferOld[x, y] = (int)random.NextDouble() * 3;
                }
            }
        }
    }
    ...
    public void SmoothMap()   {
        for (int x = 0; x < gridSize; x++) {
            for (int y = 0; y < gridSize; y++) {
                ...
                int surroundingByTheSameType = GetSurroundingByTheSameTypeCount(x, y, bufferOld[x, y]);

                //Use some smoothing rules to generate caves
                if (surroundingByTheSameType > 4) {
                    bufferNew[x, y] = 1;
                } else if (surroundingByTheSameType == 4) {
